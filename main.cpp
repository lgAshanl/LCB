...
string BigToStr(bignum a){
    ...
    while ((k >= a.floatpoint) && (trigger==0)){
        if (k >= a.floatpoint) cout << "This string " << k << " " << a.floatpoint << " _\n";
        if (a.values[k]!=0){
            //cout << a.values[k] << " " << k << "!!!\n";
            trigger = 3;
            LengthOfRes = SIGNS * k;
...
bignum StrToBig(string s){
    ...
    int mestoznaka = 0;//mestoznaka - 1, если есть знак, иначе 0
    ...
    buf = s.length()-mestoznaka; //buf - кол-во цифр
    ...
    memset(c.values, 0, sizeof(digit) * c.length);
    /*
    ...
    */
    ...
    //Заполняем целую часть
    pow = 1;
    for(long long i = t; i >= mestoznaka; i--){
        c.values[current] = c.values[current] + (s[i] - '0') * pow;
        pow = pow * 10;
        if((t-i+1) % SIGNS == 0){
            //переход на новую "пачку" чисел
...
bignum karatsuba(bignum a, bignum b) {
    ...
    if (a.length < MIN_LENGTH_FOR_KARATSUBA) { //если число короче то применять наивное умножение
        ...
        for (unsigned long long i = 0; i < a.length; ++i)
            for (unsigned long long j = 0; j < b.length; ++j) {
                ...
                */
                if (product.values[i + j] >= 10000000000000000){
                product.values[i + j] -= 10000000000000000;
...
string DllMul(string a,string b){
    ...
    res.floatpoint = x.floatpoint + y.floatpoint;

    //bigLog("DllMul", "Before BigToStr" , res);
